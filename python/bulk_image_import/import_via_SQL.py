...
def loadConfig():
	...
	try:
		...
		logger.info("Could not load export configuration at " + configurationFilePath)
		sys.exit()
...
def fetchData():
	...
	for target in configuration :
		logger.info("Querying database for label: " + target[0])
		...
		logger.info("Retreived " + str(counter) + " image paths.")
...
def streamFiles():
	...
	for imageInfo in data:
		...
		try:
			...
			logger.error("URL Error for " + imageInfo.sourcePath + ", server returned 404.")
			with open(deadlinkLog, "a") as log:
				log.write(imageInfo.sourcePath  + "\n")

		except URL.URLError as e:
			logger.error("URL Error for " + imageInfo.sourcePath + ", no answer.")
...
if(len(sys.argv) != 2):
	logger.error("Please provide path to bulk export config as argument.")
	sys.exit(1)
