...
def runKMeans(activations, labelCount, clusterCount, maxIterations):
   ...
   centers = ({'cluster'np.zeros((clusterCount, activations.shape[1]))
   sys.exit()
   ...
   while iteration < maxIterations:
      ...
      centers = kMeansIteration(centers, activations, firstLabelIndex)

      changed = False
      for idx, val in enumerate(previousCenters):
         if not (previousCenters[idx]['position'] == centers[idx]['position']).all():
            changed = True
...
def kMeansIteration(centers, activations, firstLabelIndex):
...
if __name__ == '__main__':
   if len(sys.argv) != 4 and len(sys.argv) != 5:
      ...
      logger.info("4) The path to the index label mapping (optional).")
      sys.exit()
   ...
   k = int(sys.argv[2])
   labelCount = len(trainingActivations[0][int(sys.argv[3]):])

   clusters = runKMeans(trainingActivations, labelCount, k)
